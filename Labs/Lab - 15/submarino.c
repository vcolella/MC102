...
int main(){
	...
	scanf("%d %d %d", &L, &submarino.W, &submarino.H);/*Le as dimensoes do submarino e o numero de pontos a serem fornecidos*/
	...
	for(i=1;i<=L;i++){
		scanf("%f %f", &pontos[i].x, &pontos[i].y);/*Le os pontos fornecidos*/

	}

	for(i=0;i<15;i++){
		scanf(" %c", &comando);/*Le um comando*/
